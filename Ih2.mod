...
COMMENT
    ...
    *Note: the code may produce segmentation faults if state values become
    ...
    Written by Martynas Dervinis @ Cardiff University, 2015.
...
NEURON {
    ...
    USEION ca READ cai
    RANGE ghbar, V_half, s, tau_min, shift, ginc, cainf, k2, cac, nca, k4, Pc, nexp
}
...
PROCEDURE rates(v(mV), cai(mM)) {
    ...
    caInc = cai - cainf     : Modulation Ca2+ relative to the baseline concentration.
                            : Makes easier switching between mudulated and regular versions of the model
    if (caInc < 0) {
    	caInc = 0
    }
    k1 = k2 * (caInc/cac)^nca
